...
public class JobTypeServiceImpl implements JobTypeService {
    ...
    @Override
    public boolean save(Integer jobDirectionId, JobTypeDTO jobTypeDTO) {
        // 检测工作方向和工作类别合法性
        // 工作方向id存在,工作类别名称不重复
        JobDirection jobDirection = jobDirectionDao.getById(jobDirectionId);
        JobType testType = jobTypeDao.getByName(jobTypeDTO.getName());
        if (jobDirection != null && testType == null) {
            // 为增加类别后获取自增ID,进一步转换为JobType
            JobType jobType = new JobType(jobTypeDTO.getName());
            jobTypeDao.save(jobType);
            jobTypeDao.saveDirectionType(jobDirectionId, jobType.getId());
            return true;
        }
        return false;
    }
